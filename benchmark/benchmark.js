...
const prefilledSBT = new SBT((a, b) => a - b);
rvalues.forEach((v) => prefilledSBT.add(v));
...
new Benchmark.Suite(`Insert (x${N})`, options)
  ...
  .add('SBT (current)', () => {
    const tree = new SBT();
    for (let i = 0; i < N; i++) tree.add(rvalues[i]);
  })
  .run();
...
new Benchmark.Suite(`Random read (x${N})`, options)
  ...
  .add('SBT (current)', () => {
    for (let i = N - 1; i; i--) prefilledSBT.find(rvalues[i]);
  })
  .run();
...
new Benchmark.Suite(`Remove (x${N})`, options)
  ...
  .add('SBT (current)', () => {
    for (let i = N - 1; i; i--) prefilledSBT.remove(values[i]);
  })
  .run();
